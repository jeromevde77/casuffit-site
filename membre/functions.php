...
function envoyerLienMagique($db, $membre) {
    ...
    if (!empty(BREVO_API_KEY)) {
        return envoyerViaBrevo($membre['email'], trim($membre['prenom'].' '.$membre['nom']), "Votre accès espace membre — Ça suffit !", $html, $text);
    }
    return envoyerViaSMTP($membre['email'], trim($membre['prenom'].' '.$membre['nom']), "Votre accès espace membre — Ça suffit !", $html, $text);
}
